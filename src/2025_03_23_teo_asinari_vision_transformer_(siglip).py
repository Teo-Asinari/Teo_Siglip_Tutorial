# -*- coding: utf-8 -*-
"""2025-03-23_Teo_Asinari_Vision_Transformer (SIGLIP).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1MeGsw75i7deZ__5plPMPu3V2f5ixkPDZ
"""
...
# SiglipVisionModel is the fully pretrained model weights from hugging face

processor = AutoProcessor.from_pretrained("google/siglip-base-patch16-224")
# the name means image patch size 16 and images of size 224x224
  ...
  preprocess = transforms.Compose(
      [transforms.Resize((image_size, image_size)),
       ...
       transforms.Normalize(
           mean=[0.485, 0.456, 0.406], # These constants are from ImageNet.
           std=[0.229, 0.224, 0.225] # These constants are from ImageNet.
       )]
  )

  image_tensor = preprocess(image)
  # (3, 224, 224) --> unsqueeze --> (1, 3, 224, 224)
  # unsqueeze allows use to add a size-1 batch dimension to
  # allow the transformer to consume it.
  image_tensor = image_tensor.unsqueeze(0)
  return image_tensor

image_tensor = preprocess_image(img)

# Polyanna 2 Siglip parameter constants below:
embed_dim = 768 # EAch image patch gets transformed into a 1-768 embedding vector
patch_size = 16
image_size = 224
num_patches = (image_size // patch_size) ** 2

with torch.no_grad(): # We are not training and gradients are not saved for efficiency
...
position_embedding = nn.Embedding(num_patches, embed_dim) # Randomly initialized embedding matrix that needs to be trained.
position_ids = torch.arange(num_patches).expand((1,-1))
position_ids.shape

# after flatten (1, 768, 196)
embeddings = patches.flatten(start_dim=2, end_dim=3)
# (1, 768, 196) -> (1, 196, 768)
...
# it should look random/noisy since the weights are initialized to random values
#Visualize all the patch embeddings
...
vision_model.eval() # this is the model from the hugging face checkpoint
...
"""## Hugging face checkpoint embeddings
They are uniform and smooth. They have occasional blips, which according to [the instructor](https://youtu.be/4XgDdxpXHEQ?t=1563) are representations of learned relationships from the model training. It's still not clear to me exactly why the mostly uniform embeddings is desireable.
"""
...
class SiglipVisionEmbeddings(nn.Module):
  ...
  def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
    ...
    embeddings = patch_embeds.flatten(start_dim=2, end_dim=3) # or end_dim=-1 in his example
    embeddings = embeddings.transpose(1, 2)
...
class Head(nn.Module):
  ...
  def __init__(self, n_in, n_head, context_lengths):
    super().__init__()
...
"""# Hugging face style implementation (to use their weights)"""

# Hugging face style implementation
@dataclass
class SiglipVisionConfig:
  num_channels: int = 3
  image_size: int = 224
...
class SiglipAttention(nn.Module):
  def __init__(self, config: SiglipVisionConfig):
    ...
    # This implementation uses a single projection layer (as opposed to
  ...
  def forward(self, hidden_states):
    # the hidden states are embeddings of the patches, so (batch_size, num_patches, embed_dim)
    B, T, C = hidden_states.shape
    q_states = self.q_proj(hidden_states)
    k_states = self.k_proj(hidden_states)
    v_states = self.v_proj(hidden_states)

    # Below, we split our large embedding vector and split it into num_heads pieces
    ...
    # Reshape everything to previous dimensions (like previous implementation)
...
"""# Sanity checking we loaded HuggingFace weights in correctly"""
...
"""# Assembling the full vision transformer"""
...
class SiglipVisionEmbeddings(nn.Module):
  def __init__(self, config: SiglipVisionConfig):
    ...
    self.patch_embedding = nn.Conv2d(
        ...
        out_channels = self.embed_dim,
        kernel_size = self.patch_size,
        stride = self.patch_size, # no overlapping since stride = kernel_size
        padding="valid"
    )

    self.num_patches = (self.image_size // self.patch_size) ** 2 # the images are square so this is a safe assumption
    self.num_positions = self.num_patches
    self.position_embedding = nn.Embedding(self.num_positions, self.embed_dim)
    self.register_buffer("position_ids", torch.arange(self.num_positions).expand((1, -1)), persistent=False) # this is a buffer for position ids
...
"""# OK, so we have to create 12 encoder layers, we just defined one with a single class"""
...
# checking if our keys are matching correctly
# (to see if we loaded the weights properly)

hf_state_dict = vision_model.state_dict()
out_state_dict = siglip.state_dict()
